...
def check_drinks(drink_name: str, drink_ingredients: list[str]) -> str:
    ...
    return drink_name + ' Mocktail' if set(drink_ingredients).isdisjoint(ALCOHOLS) \
...
def categorize_dish(dish_name: str, dish_ingredients: set[str]) -> str:
    ...
    for index, category in enumerate(categories):
        if dish_ingredients.issubset(category):
            return f'{dish_name}: {cat_names[index]}'
...
def compile_ingredients(dishes: list[set[str]]) -> set[str]:
    ...
    for ingredients in dishes:
        master = master.union(ingredients)
...
def singleton_ingredients(dishes: list[set[str]], intersection: set[str]) -> set[str]:
    ...
    for dish in dishes:
        # singleton = dish.difference(singleton).union(singleton.difference(dish))
        singleton = singleton.union(dish.difference(intersection))

    return singleton#.difference(intersection)
