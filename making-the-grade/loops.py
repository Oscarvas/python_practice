...
def letter_grades(highest: int):
    ...
    grades = [lowest_grade] # D threshold

    while lowest_grade + threshold < highest:
        lowest_grade += threshold
        grades.append(lowest_grade)
...
def student_ranking(student_scores: list[int], student_names: list[str]) -> list[str]:
    ...
    assert len(student_scores) == len(student_names), "Number of scores and names must match."
